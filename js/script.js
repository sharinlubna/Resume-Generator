...
function addskill() {
  ...
  function addex() {
    const head = document.createElement('div');
    document.getElementById("experiance").appendChild(head);
    head.innerHTML = ('<div class="edublock"><span><input type="checkbox" class="input-checkbox"></span><span class="education-head" contenteditable="true">YOUR EXPERIANCE</span><div ><span contenteditable="true">Company name</span> - <span contenteditable="true">From - To</span></div></div>');
    saveresume();
  }
  function remex(event) {
    let val = 0;
    let empty = true;
    const allInputCheckboxes = event.target.parentElement.getElementsByClassName("input-checkbox");
    const array = Array.from(allInputCheckboxes);
...
//   this function is to change profile pic but when downloading the pdf it doesn't shows
...
// this function shows the profile when downloaded
  ...
  function loadFile(event) {
    ...
    reader.onload = function () {
        ...
        img.onload = function () {
            ...
            // Now, call the function to generate PDF
            // printpdf();
        };
    };
